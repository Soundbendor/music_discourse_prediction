...
for ds in datasets.keys():
    for src in datasets[ds]:
        ...
        print(len(df.dropna(subset=['body'])['query_index'].unique()))

        df_total_results.loc[f'{ds}_{src}', 'Total Songs'] = len(
            df['query_index'].unique())
        df_total_results.loc[f'{ds}_{src}', 'Total Songs  w/ comments'] = len(
            df.dropna(subset=['body'])['query_index'].unique())
        ...
        # df[['query_index', 'body']].groupby('query_index').agg({'body': lambda x: x.apply(lambda y: print(y.split()))})
        ...
        df_ratio_results.loc[f'{ds}_{src}', 'Words per Song'] = df[['query_index', 'body']].groupby(
            'query_index').agg({'body': lambda x: sum(x.apply(lambda y: len(str(y).split())))}).mean()['body']
        ...
        df_avg_results.loc[f'{ds}_{src}', 'Word Avg.'] = sum(
            [len(str(x).split()) for x in df['body']]) / len(df['query_index'].unique())
        df_avg_results.loc[f'{ds}_{src}', 'Word Std.'] = np.std(
            [len(str(x).split()) for x in df['body']])

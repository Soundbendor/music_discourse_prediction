...
class YoutubeBot(CommentMiner):
    ...
    def process_submissions(self, s_result: dict) -> Submission:
        ...
        return Submission(
            ...
            lang = s_lang.lang,
            lang_p = s_lang.prob,
            url = base_url + s_result['id']['videoId'],
            id = s_result['id']['videoId'],
            score = self._get_video_score(v_resource),
            n_comments = v_resource['statistics']['commentCount'],
            subreddit = v_resource['snippet']['channelTitle'],
            comments = list(map(self.process_comments, self.yt_client.get_comments(s_result['id']['videoId'])))
        )

...
class ExperimentFactory:
    ...
    def _get_arg(self, key) -> dict:
        ...
        return {k: v.strip() for k, v in table.items()}
    ...
    def get_param_grid(self) -> list:
        pgrid_args = self._get_arg('PARAM_GRID')['params']
        print(ast.literal_eval(pgrid_args.strip('\n').strip('"')))
        return(ast.literal_eval(pgrid_args.strip('\n').strip('"')))
    ...
    def get_model(self) -> skl.base.BaseEstimator:
        ...
        supported_models = {
            ...
            'randomforestregressor': RandomForestRegressor(n_jobs=-1, random_state=128),
            'kneighborsregressor': KNeighborsRegressor(),
            'mlpregressor': MLPRegressor(random_state=128),
            'randomforestclassifier': RandomForestClassifier(n_jobs=-1, random_state=128),
            'adaboost': AdaBoostRegressor(base_estimator=DecisionTreeRegressor(), random_state=128),
            'lightgbm': HistGradientBoostingRegressor(random_state=128),
            'svm': SVR()
        }
    ...
    def get_feature_selection_strategy(self):
        ...
        supported_selection_methods = {
            ...
            'f_regression': SelectPercentile(score_func=f_regression, percentile=float(percent)),
            'mutual_info_regression': SelectPercentile(score_func=mutual_info_regression, percentile=float(percent)),
            'none': None
        }

    ...
    return (
        pd.Series(
            filter(
                ...
                map(lemmatizer.lemmatize, nltk.word_tokenize(rx.sub("", comment))),
            )
        )
        .value_counts()
        .reset_index()
...
def vectorize_comment(x: pd.Series, wordlist: pd.DataFrame):
    c_vec = (
        pd.concat(list(x), axis=0, ignore_index=True)
...
def load_emolex(path: str) -> pd.DataFrame:
    return (
        pd.read_csv(
            path, names=["Word", "Emotion", "Association"], skiprows=1, sep="\t"
        )
        .groupby(["Word"])
        .apply(lambda x: x.set_index("Emotion")["Association"])
        .reset_index()
    )
...
def load_mpqa(path: str) -> pd.DataFrame:
    return (
        pd.read_csv(path, names=["Word", "Sentiment"], skiprows=0)
        .drop_duplicates(subset="Word")
        .replace(
            {"Sentiment": {"positive": 1, "negative": -1, "neutral": 0, "both": 0}}
        )
    )
...
loaders = {
    ...
    "EmoLex": load_emolex,
    "EmoVAD": lambda x: pd.read_csv(
        x, names=["Word", "Valence", "Arousal", "Dominance"], skiprows=1, sep="\t"
    ),
    "EmoAff": load_emoaff,
    "HSsent": None,
    "MPQA": load_mpqa,
...
def get_features(wlist, args, song_groups, df) -> pd.DataFrame:
    ...
    emo_word_stats = song_groups["body"].apply(lambda x: vectorize_comment(x, wordlist))
...
def gen_features(wlist, args) -> None:
    ...
    df = get_song_df(args.input).dropna(subset=["body"]).pipe(tokenize_comments)
    ...
    df["n_comments"] = df0.groupby(["query_index"]).apply(
        lambda x: x.groupby(["submission.id"])["submission.n_comments"]
        .aggregate(lambda x: x.iloc[0])
        .aggregate("sum")
    )
    df["n_words"] = df0.groupby(["query_index"])["body"].aggregate(
        lambda x: sum(x.apply(lambda y: y["Count"]))
    )
    print(df)
    ...
    if wlist == "All":
        feature_space = pd.concat(
            [get_features(wordlist, args, song_groups, df) for wordlist in wlists],
            axis=1,
        )
        print(feature_space)
